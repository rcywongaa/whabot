...
template <typename T>
ConstraintForce<T>::ConstraintForce(
...
template <typename T>
void ConstraintForce<T>::DoCalcAndAddForceContribution(
...
template <typename T>
T ConstraintForce<T>::CalcPotentialEnergy(
    const systems::Context<T>&,
...
template <typename T>
T ConstraintForce<T>::CalcConservativePower(
    const systems::Context<T>&,
    const internal::PositionKinematicsCache<T>& pc,
...
template <typename T>
T ConstraintForce<T>::CalcNonConservativePower(
    const systems::Context<T>&,
    const internal::PositionKinematicsCache<T>& pc,
    const internal::VelocityKinematicsCache<T>& vc) const {
  // The rate at which the length of the spring changes.
...
ConstraintForce<T>::TemplatedDoCloneToScalar(
  ...
  auto spring_damper_clone = std::make_unique<ConstraintForce<ToScalar>>(
...
ConstraintForce<T>::DoCloneToScalar(
...
template <typename T>
T ConstraintForce<T>::SafeSoftNorm(const Vector3<T> &x) const {
  using std::sqrt;
...
template <typename T>
T ConstraintForce<T>::CalcLengthTimeDerivative(
    const internal::PositionKinematicsCache<T>& pc,
    const internal::VelocityKinematicsCache<T>& vc) const {
  const math::RigidTransform<T>& X_WA = pc.get_X_WB(bodyA().node_index());
  const math::RigidTransform<T>& X_WB = pc.get_X_WB(bodyB().node_index());
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
    class ::drake::multibody::ConstraintForce)
