...
def derivs(state, tau234):
    ...
    state_d[4] = tau1 / find_I_wrt_1(theta1, theta2, theta3)
    state_d[5] = tau234[0] / find_I_wrt_2(theta2, theta3)
    state_d[6] = tau234[1] / find_I_wrt_3()
...
if __name__ == "__main__":
    ...
    for i in range(NUM_TIME_STEPS-1):
        tau234_over_time[i] = mp.NewContinuousVariables(3, "tau234_%d" % i)
    ...
    mp.AddConstraint(state_over_time[0, 2] <= np.pi/2.0)
    mp.AddConstraint(state_over_time[0, 2] >= 0.0)
    ...
    torques_over_time = result.GetSolution(tau234_over_time)
